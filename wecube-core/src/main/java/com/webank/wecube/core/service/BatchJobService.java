...
import com.webank.wecube.core.jpa.BatchJobHostRepository;
import com.webank.wecube.core.jpa.BatchJobRepository;
import com.webank.wecube.core.service.batchjob.CreateBatchJobRequest;
import com.webank.wecube.core.service.batchjob.CreateBatchJobResponse;
import com.webank.wecube.core.service.batchjob.GetContextRequest;
import com.webank.wecube.core.service.batchjob.SearchTextProcessorRequest;
import com.webank.wecube.core.support.plugin.PluginServiceStub;
import com.webank.wecube.core.support.plugin.dto.PluginRequest;
import com.webank.wecube.core.support.plugin.dto.PluginResponse;
import com.webank.wecube.core.support.plugin.dto.PluginRunScriptOutput;
...
public class BatchJobService {
    ...
    public CreateBatchJobResponse createBatchJob(CreateBatchJobRequest request) throws IOException {
        ...
        String s3Url = new S3Client(s3properties.getEndpoint(), s3properties.getAccessKey(), s3properties.getSecretKey())
                .uploadBinaryToS3(S3_BUCKET_NAME_FOR_SCRIPTS, s3KeyName, content.getBytes());
    ...
    public void executeBatchJob(Integer batchId) throws IOException {
        ...
        List<PluginInstance> instances = pluginInstanceService.getRunningPluginInstances(pluginProperties.getPluginPackageNameOfDeploy());
        ...
        for (int i = 0; i < hosts.size(); i++) {
            inputs.add(ImmutableMap.<String, Object>builder()
                    .put("target", hosts.get(i).getHostIp())
        ...
        PluginResponse.ResultData<PluginRunScriptOutput> response = pluginServiceStub.callPluginRunScript(pluginInstanceService.getInstanceAddress(instances.get(0)), request);
        ...
        for (int i = 0; i < hosts.size(); i++) {
            ...
            s3Url = new S3Client(s3properties.getEndpoint(), s3properties.getAccessKey(), s3properties.getSecretKey())
                    .uploadBinaryToS3(S3_BUCKET_NAME_FOR_SCRIPT_OUTPUT, s3KeyName, outputs.get(i).getDetail().getBytes());
    ...
    public Object searchTextProcessor(SearchTextProcessorRequest req) {
        ...
        for (int i = 0; i < hosts.size(); i++) {
            ...
            inputs.add(ImmutableMap.<String, Object>builder()
                    .put("target", dbHosts.get(0).getHostIp())
                    .put(CONSTRANT_ENDPOINT, dbHosts.get(0).getOutputUrl())
                    .put("pattern", req.getPattern())
                    .put(CONSTRANT_ACCESS_KEY, s3properties.getAccessKey())
                    .put(CONSTRANT_SECRET_KEY, s3properties.getSecretKey()).build());
        }
        request.setInputs(inputs);
