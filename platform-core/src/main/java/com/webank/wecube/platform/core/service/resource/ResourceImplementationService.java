...
public class ResourceImplementationService {
    ...
    public void createItems(Iterable<ResourceItem> items) {
        for (ResourceItem item : items) {
            if (item.getResourceServer() == null) {
                throw new WecubeCoreException("3248",String.format("Failed to create resource item [%s] as resource server is missing.", item));
            }

            ResourceItemService service = itemServices.get(ResourceItemType.fromCode(item.getType()));
            if (service == null) {
                throw new WecubeCoreException("3249",String.format("No service for the creating of resource type [%s].", item.getType()));
            }
    ...
    public void updateItems(Iterable<ResourceItem> items) {
        for (ResourceItem item : items) {
            if (item.getResourceServer() == null) {
                throw new WecubeCoreException("3250",String.format("Failed to update resource item [%s] as resource server is missing.", item));
            }

            ResourceItemService service = itemServices.get(ResourceItemType.fromCode(item.getType()));
            if (service == null) {
                throw new WecubeCoreException("3251",String.format("No service for the updating of resource type [%s].", item.getType()));
            }
    ...
    public void deleteItems(Iterable<ResourceItem> items) {
        for (ResourceItem item : items) {
            if (item.getResourceServer() == null) {
                throw new WecubeCoreException("3252",String.format("Failed to update resource item [%s] as resource server is missing.", item));
            }

            ResourceItemService service = itemServices.get(ResourceItemType.fromCode(item.getType()));
            if (service == null) {
                throw new WecubeCoreException("3253",String.format("No service for the deleting of resource type [%s].", item.getType()));
            }
