...
public class WorkflowEngineService {
    ...
    private void tryEmitSignalEvent(String eventName, EventSubscription signalEventSubscription, String resultCode,
        ...
        for (int times = 0; times <= 10; times++) {
            ...
            try {
                ...
                log.info("Errors while sending signal " + resultCode + " -" + e.getMessage());
                log.info(
    ...
    public ProcessInstance startProcessInstance(String processDefinitionId, String processInstanceKey) {
        ...
        if (instance == null) {
            log.warn("Failed to create process instance with id {} and key {}", processDefinitionId,
                    processInstanceKey);
            throw new WecubeCoreException("3200",
                    String.format("Failed to create process instance with id %s and key %s.", processDefinitionId,
                            processInstanceKey),
    ...
    public ProcDefOutline readProcDefOutlineFromXmlData(String xmlData) {
        ...
        populateFlowNodes(pdo, startEvent, null);
    ...
    protected void populateFlowNodes(ProcDefOutline outline, FlowNode flowNode, ProcFlowNode previous) {
        Collection<FlowNode> previousFlowNodes = flowNode.getPreviousNodes().list();

        for (FlowNode fn : previousFlowNodes) {
            ProcFlowNode childPfn = outline.findFlowNode(fn.getId());
            if (childPfn == null) {
                populateFlowNodes(outline, fn, null);
            }
        }
